...
- [ ] State machine handles: INVITE_VALIDATION → PROFILE_COLLECTION → GITHUB_REQUEST → GITHUB_VERIFICATION → REPUTATION_CALCULATION → WALLET_REQUEST → COMPLETION
...
- `plugin-perk-os/src/actions/register.ts` - Main action
- `plugin-perk-os/src/utils/registrationStateMachine.ts` - State machine logic
- `plugin-perk-os/src/__tests__/registration.test.ts` - Unit tests
- `plugin-perk-os/src/__tests__/e2e/registrationFlow.e2e.ts` - E2E tests
- `plugin-perk-os/src/index.ts` - Register action (modify)
